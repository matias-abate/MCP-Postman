...
// Config del servidor
// =====================
const server = new Server({ name: "postman-mcp-server", version: "1.0.0" }, { capabilities: { tools: {} } });
// =====================
// Postman API
// =====================
const POSTMAN_API_BASE = "https://api.getpostman.com";
let postmanApiKey = process.env.POSTMAN_API_KEY ?? "";
async function makePostmanRequest(endpoint, method = "GET", data) {
    if (!postmanApiKey) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "create_workspace") {
        const { name: wname, type = "personal", description = "" } = (args ?? {});
        if (!wname)
    ...
    if (name === "create_mock_server") {
        const { name: mname, collectionId, environmentId } = (args ?? {});
        if (!mname || !collectionId)
    ...
    if (name === "create_documentation") {
        const { name: dname, /* collectionId, */ content = "" } = (args ?? {});
        if (!dname)
            throw new Error("name es requerido");
        const payload = {
            collection: {
